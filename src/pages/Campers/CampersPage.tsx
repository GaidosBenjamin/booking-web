...
const GRUP_MIC_CODES = ['GM1', 'GM2', 'GM3', 'GM4', 'GM5', 'GM6', 'GM7', 'GM8', 'GM9'] as const;
...
const emptyForm: CamperFormData = { firstName: '', lastName: '', grade: '', gender: 'male' };
...
export default function CampersPage() {
  ...
  const grupMicLabel = (code: string) => {
    const key = `campers.form.grupMicOptions.${code}`;
    const label = t(key);
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateCamperRequest }) => updateCamper(id, data),
  ...
  const toPayload = (): UpdateCamperRequest => ({
  ...
  const handleSaveNew = () => {
    if (!formData.firstName || !formData.lastName || !formData.grade) {
      showToast(t('campers.toasts.fillRequired'), 'error');
      return;
    }
    createMut.mutate(toPayload() as CreateCamperRequest);
  };

  const handleSaveEdit = (camper: CamperResponse) => {
    updateMut.mutate({ id: camper.id, data: toPayload() });
  };
  ...
  const renderForm = (onSave: () => void, onRemove?: () => void) => (
    <div className="px-6 pb-8 pt-2 space-y-5">
      ...
      <div className="space-y-1.5">
        <label className="font-label text-xs font-semibold text-on-surface-variant ml-1">{t('campers.form.grupMic')} <span className="text-error">*</span></label>
        <div className="relative">
          <select value={formData.grade} onChange={e => setFormData(p => ({ ...p, grade: e.target.value }))} className="w-full h-12 bg-surface-container-high border-none rounded-lg px-3 text-base text-primary font-medium focus:ring-2 focus:ring-secondary/20 appearance-none pr-10">
            <option value="" disabled>{t('campers.form.grupMicPlaceholder')}</option>
            {GRUP_MIC_CODES.map(code => (
              <option key={code} value={code}>{grupMicLabel(code)}</option>
            ))}
          </select>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 items-center mt-1 flex-wrap">
                        <span className="bg-surface-container-low text-on-surface-variant text-[10px] font-bold px-2 py-0.5 rounded-full">{grupMicLabel(camper.grade)}</span>
